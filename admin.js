$(document).ready(function () {
  ...
  function viewProducts() {
    $.ajax({
      ...
      success: function (response) {
        ...
        // Event listener for stock in / stock out
        $(".stock-in-out").on("click", function (e) {
          e.preventDefault();
          stockInStockOut(this.dataset.id);
        });
  ...
  function stockInStockOut(productId) {
    $.ajax({
      url: `../stocks/stock-in-stock-out.html`,
      type: "GET",
      datatype: "html",
      success: function (view) {
        $(".modal-container").empty().html(view); // Load the modal view
  ...
  function updateStock(productId) {
    $.ajax({
      type: "POST",
      url: `../stocks/stock-in-out.php?product_id=${productId}`,
      data: $("form").serialize(),
      dataType: "json",
      success: function (response) {
